...
function FloralDivider() {
  return (
    <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", marginVertical: 4 }}>
      <Text style={{ color: "#b8972e", fontSize: 11, letterSpacing: 4 }}>
        {"\u2726 \u2767 \u2726"}
      </Text>
    </View>
  );
...
export default function HomeScreen() {
  ...
  const { setPendingArtistId, bookings } = useApp();
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#0a0a0a" }}>
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} bounces={false}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 20, paddingTop: 16, paddingBottom: 8 }}>
          <View>
            <Text style={{ color: "#b8972e", fontSize: 10, fontWeight: "800", letterSpacing: 4, textTransform: "uppercase" }}>Est. 2012</Text>
            <Text style={{ color: "#fff", fontSize: 22, fontWeight: "900", letterSpacing: 4, textTransform: "uppercase" }}>INK STUDIO</Text>
            <Text style={{ color: "#b8972e", fontSize: 10, letterSpacing: 4 }}>{"\u2726 \u2767 \u2726"}</Text>
          </View>
          <View style={{ flexDirection: "row", gap: 8, alignItems: "center" }}>
            <TouchableOpacity
              onPress={() => router.push("/history")}
              style={{ borderColor: "#b8972e", borderWidth: 1, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6 }}
            >
              <Text style={{ color: "#b8972e", fontSize: 10, fontWeight: "700", letterSpacing: 1 }}>HISTORICO</Text>
              {(pendingCount + upcomingCount) > 0 && (
                <View style={{ position: "absolute", top: -5, right: -5, backgroundColor: "#b8972e", borderRadius: 10, width: 16, height: 16, alignItems: "center", justifyContent: "center" }}>
                  <Text style={{ color: "#000", fontSize: 9, fontWeight: "900" }}>{pendingCount + upcomingCount}</Text>
                </View>
              )}
            </TouchableOpacity>
            <View style={{ borderColor: "#b8972e", borderWidth: 1, borderRadius: 20, paddingHorizontal: 10, paddingVertical: 5 }}>
              <Text style={{ color: "#b8972e", fontSize: 10, fontWeight: "700", letterSpacing: 2 }}>SP / BR</Text>
            </View>
          </View>
        </View>
        ...
        <View style={{ marginHorizontal: 16, marginTop: 12, borderRadius: 16, overflow: "hidden", height: 420 }}>
          <ImageBackground source={{ uri: "https://picsum.photos/seed/tattoohero/800/900" }} style={{ flex: 1 }} resizeMode="cover">
            <View style={{ flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(0,0,0,0.55)" }}>
              <View style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, backgroundColor: "#b8972e" }} />
              <View style={{ padding: 24 }}>
                <View style={{ backgroundColor: "#b8972e", alignSelf: "flex-start", paddingHorizontal: 12, paddingVertical: 4, borderRadius: 4, marginBottom: 4 }}>
                ...
                <Text style={{ color: "#a1a1aa", fontSize: 13, marginTop: 12, lineHeight: 20 }}>{"Cada traco, um simbolo. Cada peca, unica.\nAgende sua sessao com os melhores artistas."}</Text>
                <TouchableOpacity
                  style={{ marginTop: 20, backgroundColor: "#b8972e", borderRadius: 12, paddingVertical: 16, alignItems: "center", shadowColor: "#b8972e", shadowOpacity: 0.4, shadowRadius: 12, elevation: 8 }}
                  onPress={() => goToBooking()}
                  activeOpacity={0.85}
                >
        ...
        <View style={{ flexDirection: "row", marginHorizontal: 16, marginTop: 16, backgroundColor: "#111111", borderRadius: 16, borderWidth: 1, borderColor: "#1f1f1f", overflow: "hidden" }}>
          {[{ value: "12+", label: "Anos" }, { value: "4.000+", label: "Tattoos" }, { value: "4", label: "Artistas" }, { value: "\u2605 4.9", label: "Avaliacao" }].map((stat, i, arr) => (
            <View key={stat.label} style={{ flex: 1, alignItems: "center", paddingVertical: 16, borderRightWidth: i < arr.length - 1 ? 1 : 0, borderRightColor: "#1f1f1f" }}>
              <Text style={{ color: "#b8972e", fontSize: 16, fontWeight: "900" }}>{stat.value}</Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingLeft: 20, paddingRight: 8 }}>
          {TATTOO_STYLES.map((style) => (
            <TouchableOpacity key={style.id} style={{ marginRight: 12, backgroundColor: "#111111", borderWidth: 1, borderColor: "#2a2310", borderRadius: 16, paddingHorizontal: 20, paddingVertical: 16, alignItems: "center", minWidth: 100 }} activeOpacity={0.75}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingLeft: 20, paddingRight: 8 }}>
          {GALLERY_IMAGES.map((img) => (
            <View key={img.id} style={{ marginRight: 12, borderRadius: 16, overflow: "hidden", width: width * 0.6, height: 220 }}>
              ...
              <View style={{ position: "absolute", top: 0, right: 0, width: 24, height: 24, backgroundColor: "#b8972e", borderBottomLeftRadius: 10 }} />
            </View>
          ))}
        </ScrollView>
            ...
            {ARTISTS.map((artist) => (
              <View key={artist.id} style={{ backgroundColor: "#111111", borderWidth: 1, borderColor: "#2a2310", borderRadius: 16, padding: 16, marginBottom: 12, flexDirection: "row", alignItems: "center" }}>
                <View style={{ position: "relative" }}>
                  ...
                  <View style={{ position: "absolute", bottom: -2, right: -2, width: 16, height: 16, backgroundColor: "#b8972e", borderRadius: 8, borderWidth: 2, borderColor: "#0a0a0a" }} />
                </View>
                <View style={{ flex: 1, marginLeft: 16 }}>
                  <Text style={{ color: "#fff", fontWeight: "900", fontSize: 15 }}>{artist.name}</Text>
                  <Text style={{ color: "#b8972e", fontSize: 11, fontWeight: "600", marginTop: 2 }}>{artist.specialty}</Text>
                  <Text style={{ color: "#a1a1aa", fontSize: 11, marginTop: 4 }}>{artist.experience} · {artist.instagram}</Text>
                </View>
                <TouchableOpacity style={{ backgroundColor: "#b8972e", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 10 }} onPress={() => goToBooking(artist.id)} activeOpacity={0.8}>
        ...
        <View style={{ marginTop: 32, paddingHorizontal: 20 }}>
          ...
          <View style={{ marginTop: 12 }}>
            {[
              { step: "01", title: "Preencha o Formulario", desc: "Informe o estilo, tamanho, local e envie sua referencia." },
              { step: "02", title: "Receba seu Orcamento", desc: "Em ate 24h o artista entra em contato com o valor e disponibilidade." },
              { step: "03", title: "Confirme e Apareca", desc: "Aprovado o orcamento, pague 20% de caucao e sua sessao esta garantida!" },
            ].map((item, index) => (
              <View key={item.step} style={{ flexDirection: "row", marginBottom: 20 }}>
                <View style={{ alignItems: "center", marginRight: 16 }}>
                  <View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: "#b8972e", alignItems: "center", justifyContent: "center" }}>
        ...
        {/* BOTTOM CTA */}
        <View style={{ marginHorizontal: 16, marginTop: 16, marginBottom: 40, backgroundColor: "#111111", borderWidth: 1, borderColor: "#b8972e", borderRadius: 16, padding: 24, alignItems: "center", shadowColor: "#b8972e", shadowOpacity: 0.15, shadowRadius: 20, elevation: 6 }}>
          <Text style={{ color: "#b8972e", fontSize: 11, fontWeight: "700", letterSpacing: 3, textTransform: "uppercase", marginBottom: 4 }}>Pronto para a transformacao?</Text>
          <FloralDivider />
          ...
          <TouchableOpacity style={{ backgroundColor: "#b8972e", width: "100%", borderRadius: 12, paddingVertical: 16, alignItems: "center", shadowColor: "#b8972e", shadowOpacity: 0.45, shadowRadius: 12, elevation: 8 }} onPress={() => goToBooking()} activeOpacity={0.85}>
            <Text style={{ color: "#000", fontWeight: "900", fontSize: 14, letterSpacing: 3, textTransform: "uppercase" }}>{"\u2726 Agendar Agora"}</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
